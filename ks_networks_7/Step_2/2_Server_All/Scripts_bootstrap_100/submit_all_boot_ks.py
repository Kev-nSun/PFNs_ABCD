...
tmpdir = '{0}/tempfiles_matchedsamples_031324'.format(homedir,dirdate)  #It will be created if not exist    (or os.environ['TMPDIR'])
...
for psy_score in ['General_PB1']:
    print('qsub -l h_vmem={0},s_vmem={0} -N {1}{4} -R y -V -j y -b y -o ./sge_{6}/ -e ./sge_{6}/ python {2}/ks_proc_predict_boot.py {2} {3} {4} {5} {6}'.format(GB,psy_score.split('_')[1],homedir,tmpdir,network,psy_score,dirdate))
    os.system('qsub -l h_vmem={0},s_vmem={0} -N {1}{4} -R y -V -j y -b y -o ./sge_{6}/ -e ./sge_{6}/ python {2}/ks_proc_predict_boot.py {2} {3} {4} {5} {6}'.format(GB,psy_score.split('_')[1],homedir,tmpdir,network,psy_score,dirdate))
    #time.sleep(3)
