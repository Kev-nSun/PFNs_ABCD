...
#determine relative threshold for max pos and max neg weights (top 10% by magnitude)
pos_thres <- quantile(max_pos_weight[,1],prob = 0.95) #F1= 10% 0.00237, 5% 0.00288, 1% 0.00393;       F2= 10% 0.00222, 5% 0.0029, 1% 0.00484
neg_thres <- quantile(max_neg_weight[,1],prob = 0.05) #F1= 10% -0.00185, 5% -0.00225, 1% -0.000203;    F2= 10% -0.00207, 5% -0.00277, 1% -0.00483
...
writeMat(paste0(dirout,"Max_Pos_",catg,"_L_clust_5.mat"), x = names_list)
# For R hemi, ensure that both the list and its elements are named
names_list <- list()
names_list$vertex_ID_rh <- vertex_ID_rh
writeMat(paste0(dirout,"Max_Pos_",catg,"_R_clust_5.mat"), x = names_list)
...
names_list <- list()
names_list$vertex_ID_lh <- vertex_ID_lh
writeMat(paste0(dirout,"Max_Neg_",catg,"_L_clust_5.mat"), x = names_list)
# For R hemi, ensure that both the list and its elements are named
names_list <- list()
names_list$vertex_ID_rh <- vertex_ID_rh
writeMat(paste0(dirout,"Max_Neg_",catg,"_R_clust_5.mat"), x = names_list)