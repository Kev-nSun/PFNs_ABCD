...
import numpy as np
"""
syntax:  python submit_haufeAAB.py mmddyy
...
it assigns the current running dir to home_dir variable. It creates 
   ...
   result dir = <current dir>/Weight_Haufe_<mmddyy>
"""
...
# submit_ks.py has run preprocess_ks.py for all.  Here don't need to run again. If not, uncomment lines 32 & 33.
...
for outcome in ['PRS_1','PRS_2']:
    print('qsub -l h_vmem={0},s_vmem={0} -N {1}{4} -R y -V -j y -b y -o ./sge_{6}/ -e ./sge_{6}/ python {2}/ks_proc_predict_haufeAAB.py {2} {3} {4} {5} {6}'.format(GB,outcome[1],homedir,tmpdir,network,outcome,dirdate))
    os.system('qsub -l h_vmem={0},s_vmem={0} -N {1}{4} -R y -V -j y -b y -o ./sge_{6}/ -e ./sge_{6}/ python {2}/ks_proc_predict_haufeAAB.py {2} {3} {4} {5} {6}'.format(GB,outcome[1],homedir,tmpdir,network,outcome,dirdate))
    #time.sleep(3)
