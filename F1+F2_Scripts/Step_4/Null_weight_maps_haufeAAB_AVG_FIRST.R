...
dirin <- "../Step_5/Haufe_Nulls_021224/";
dirout <- "weight_map_by_vertex_HaufeAAB_All_cov_nii/nulls/"
dir.create(dirout)
  ...
  for (dir in c('A','B','C','D','E','F','G','H','I','J')) { #cycle through 10 directories
    for (fold in 0:99) { #cycle through 100 files
      ...
      for (grp in c('A','B')) {
        ...
        infile <- paste0(dirin, 'Weight_Haufe_Nulls_021224_All_cov_', dir, '/', catg, '_', grp, '_Weight_haufetrans_all_', fold,'.mat')
