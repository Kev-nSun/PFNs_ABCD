...
dirin <- "../../Step_2-Ridge_reg/4_Brain_maps/FULL/Haufe_PRS_040724/Weight_Haufe_040724/";
...
#write.csv(sum_weights, file = paste0(dirout,catg,"_sum_of_weights_HaufeAAB.csv")) #save out values

#NOT NORMED
# Using 7 colors scheme for bar plot
# bar plot for NET sum of weights
NET_sorted = rank(sum_weights[,1]);
NET_inAscend <- c()
for (t in c(1:17)) { NET_inAscend <- append(NET_inAscend,which(NET_sorted == t))}

data <- data.frame(NET = as.numeric(sum_weights[,1])) 
...
LineType <- c("dashed", "dashed", "dashed", "solid", "dashed",
              ...
              "solid", "dashed", "solid", "dashed", "dashed",
              "dashed", "solid");
...
Fig
ggsave(paste0(dirout,"NET_",catg,".tiff"), width = 19, height = 16, dpi = 600, units = "cm")


# bar plot for ABS sum of weights
ABS_sorted = rank(sum_weights[,2]);
ABS_inAscend <- c()
for (t in c(1:17)) { ABS_inAscend <- append(ABS_inAscend,which(ABS_sorted == t))}

data <- data.frame(ABS = as.numeric(sum_weights[,2])) 
...
Fig
ggsave(paste0(dirout,"ABS_",catg,".tiff"), width = 19, height = 16, dpi = 600, units = "cm")
...
#write.csv(sum_weights_norm, file = paste0(dirout,catg,"_Sum_of_weights_vert_count_norm_HaufeAAB.csv")) #save out normed values
...
BorderColor2 <- c("#FFFFFF", "#FFFFFF", "#FFFFFF", "#7499C2", "#FFFFFF",
                  ...
                  "#7499C2", "#FFFFFF", "#7499C2", "#FFFFFF", "#FFFFFF", 
                  "#FFFFFF", "#F5BA2E");
...
Fig <- ggplot(data, aes(NET_Rank, NET)) +
  ...
  scale_x_discrete(labels = as.character(NET_inAscend),limits = as.character(NET_sorted)) 
Fig
ggsave(paste0(dirout,"NET_",catg,"_norm_vert_count.tiff"), width = 19, height = 16, dpi = 600, units = "cm")
...
# BorderColor2 is for colors of bar fill 1-17 networks (in a 1-17 order). If the bar is nofill, change the code to "#FFFFFF" for that PFN. ("black" is filled with black)
BorderColor2 <- c("#E76178", "#FFFFFF", "#F5BA2E", "#7499C2", "#FFFFFF",
                  "#AF33AD", "#E443FF", "#FFFFFF", "#FFFFFF", "#AF33AD",
                  "#FFFFFF", "#E76178", "#7499C2", "#FFFFFF", "#F5BA2E", 
                  "#4E31A8", "#F5BA2E");

# the line type for the 1-17 network bars (in a 1-17 order).  If a dashed line is needed for a bar, please change the value to "dashed".
LineType <- c("solid", "dashed", "solid", "solid", "dashed",
              "solid", "solid", "dashed", "dashed", "solid",
              "dashed", "solid", "solid", "dashed", "solid",
              "solid", "solid");
...
Fig <- ggplot(data, aes(ABS_Rank, ABS)) +
  ...
  scale_x_discrete(labels = as.character(ABS_inAscend),limits = as.character(ABS_sorted)) 
Fig
ggsave(paste0(dirout,"ABS_",catg,"_norm_vert_count_FEAT_IMP_NORMED_1.tiff"), width = 19, height = 16, dpi = 600, units = "cm")
...
for (grp in c('A','B')) {
  ...
  write.csv(sum_weights, file = paste0(dirout,"Sum_of_weights_",catg,"_",grp,"_HaufeAAB.csv")) #save out values
  ...
  write.csv(sum_weights_norm, file = paste0(dirout,"Sum_of_weights_",catg,"_",grp,"_vert_count_norm_HaufeAAB.csv")) #save out normed values
  ...
  BorderColor1 <- c("#E76178", "#7499C2", "#F5BA2E", "#7499C2", "#00A131",
                    ...
                    "#7499C2", "#E76178", "#7499C2", "#00A131", "#F5BA2E", 
                    "#4E31A8", "#F5BA2E");
  if (grp == 'A') {  #based on sig testing of grp A (F1 = none, F2 = 13)
    # BorderColor2 is for colors of bar fill 1-17 networks (in a 1-17 order). If the bar is nofill, change the code to "#FFFFFF" for that PFN. ("black" is filled with black)
    BorderColor2 <- c("#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF",
                      "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF",
                      "#FFFFFF", "#FFFFFF", "#7499C2", "#FFFFFF", "#FFFFFF", 
                      "#FFFFFF", "#FFFFFF");
  } else if (grp == 'B') {  #based on sig testing of grp B (F1, F2 = none)
    # BorderColor2 is for colors of bar fill 1-17 networks (in a 1-17 order). If the bar is nofill, change the code to "#FFFFFF" for that PFN. ("black" is filled with black)
    BorderColor2 <- c("#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF",
                      "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF",
                      "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", 
  ...
  Fig <- ggplot(data, aes(NET_Rank, NET)) +
    ...
    scale_x_discrete(labels = as.character(NET_inAscend),limits = as.character(NET_sorted)) 
  Fig
  ggsave(paste0(dirout,"/NET_",catg,"_",grp,"_sig_vert_norm.tiff"), width = 19, height = 16, dpi = 600, units = "cm")
  ...
  BorderColor1 <- c("#E76178", "#7499C2", "#F5BA2E", "#7499C2", "#00A131",
                    ...
                    "#7499C2", "#E76178", "#7499C2", "#00A131", "#F5BA2E", 
                    "#4E31A8", "#F5BA2E");
  if (grp == 'A') {  #based on sig testing of grp A (F1 = 15, F2 = none)
    # BorderColor2 is for colors of bar fill 1-17 networks (in a 1-17 order). If the bar is nofill, change the code to "#FFFFFF" for that PFN. ("black" is filled with black)
    BorderColor2 <- c("#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF",
                      "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF",
                      "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", "#FFFFFF", 
  ...
  Fig <- ggplot(data, aes(ABS_Rank, ABS)) +
    ...
    scale_x_discrete(labels = as.character(ABS_inAscend),limits = as.character(ABS_sorted)) 
  Fig
  ggsave(paste0(dirout,"/ABS_",catg,"_",grp,"_sig_vert_norm.tiff"), width = 19, height = 16, dpi = 600, units = "cm")
}
