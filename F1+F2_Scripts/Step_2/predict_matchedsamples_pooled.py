...
python predict_matchedsamples_pooled.py results_matchedsamples_050224/ tempfiles_matchedsamples_040524/features_all.npy tempfiles_matchedsamples_040524/phenotypes_target.csv PRS_1 tempfiles_matchedsamples_040524/phenotypes_control.csv
python predict_matchedsamples_pooled.py results_matchedsamples_050224/ tempfiles_matchedsamples_040524/features_all.npy tempfiles_matchedsamples_040524/phenotypes_target.csv PRS_2 tempfiles_matchedsamples_040524/phenotypes_control.csv

"""

output_dir = sys.argv[1] #'homedir/results_matchedsamples_mmddyy/0_network/'
os.makedirs(output_dir,exist_ok=True)
feature_path = sys.argv[2]  #'homedir/tempfiles_matchedsamples_mmddyy/features_0.npy'
phenotype_path = sys.argv[3] #'homedir/tempfiles_matchedsamples_mmddyy/phenotypes_target.csv'
phenotype_name = sys.argv[4] #'PRS_1'
control_path = sys.argv[5] #'homedir/tempfiles_matchedsamples_mmddyy/phenotypes_control.csv'
...
#save the results
np.save('{0}/{1}_prediction_testB.npy'.format(output_dir,phenotype_name),predicted_y_B)
np.save('{0}/{1}_coefs_testB.npy'.format(output_dir,phenotype_name),coefs)
np.save('{0}/{1}_alphas_testB.npy'.format(output_dir,phenotype_name),alphas)
...
#save the results
np.save('{0}/{1}_prediction_testA.npy'.format(output_dir,phenotype_name),predicted_y_A)
np.save('{0}/{1}_coefs_testA.npy'.format(output_dir,phenotype_name),coefs)
np.save('{0}/{1}_alphas_testA.npy'.format(output_dir,phenotype_name),alphas)
...
np.save('{0}/{1}_acc_testA.npy'.format(output_dir,phenotype_name),acc_testA)
np.save('{0}/{1}_acc_testB.npy'.format(output_dir,phenotype_name),acc_testB)

# caluclate pooled pred accuracy and save it out
acc_pooled = pearsonr(np.concatenate((y_A,y_B)), np.concatenate((predicted_y_A,predicted_y_B)))[0]
mdic = {"acc_pooled": acc_pooled}
savemat("{0}/{1}_acc_pooled.mat".format(output_dir,phenotype_name), mdic)