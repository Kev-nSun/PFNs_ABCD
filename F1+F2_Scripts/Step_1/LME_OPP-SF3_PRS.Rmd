...
abcd.data.combined.toharmonize <- abcd.data.combined[,c("Factor3")]
...
abcd.data.nopfac <- subset(abcd.data.combined,select=-c(Factor3))
...
colnames(abcd.data.combined.harmonized)[58]<-"Factor3"
...
# linear mixed effects models associating OPP sub-fac with F1 separately
```{r}

summary(lmer(Factor3~PRS_1+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized))
mod1<-lmer(Factor3~PRS_1+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized)
tab_model(mod1,pred.labels = c("Intercept","PRS_1","Age","Sex"),dv.labels=c("OPP"),digits=2,digits.p=2,show.se=TRUE,show.stat=TRUE,string.stat = "t",string.se = "Std. Error",show.intercept=TRUE,show.ci = FALSE, p.adjust="none",p.style="scientific", file = paste("Associations_F1_OPP_COMBAT.doc", sep = ""))
...
# linear mixed effects models associating OPP sub-fac with F2 separately including meanFD
```{r}

summary(lmer(Factor3~PRS_2+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized))
mod1<-lmer(Factor3~PRS_2+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized)
tab_model(mod1,pred.labels = c("Intercept","PRS_2","Age","Sex"),dv.labels=c("OPP"),digits=2,digits.p=2,show.se=TRUE,show.stat=TRUE,string.stat = "t",string.se = "Std. Error",show.intercept=TRUE,show.ci = FALSE, p.adjust="none",p.style="scientific", file = paste("Associations_F2_OPP_COMBAT.doc", sep = ""))
