...
abcd.data.combined.toharmonize <- abcd.data.combined[,c("General_p")]
mod <- model.matrix(~abcd.data.combined$interview_age+abcd.data.combined$sex+abcd.data.combined$rel_family_id+abcd.data.combined$PRS_1+abcd.data.combined$PRS_2)
...
abcd.data.nopfac <- subset(abcd.data.combined,select=-c(General_p))
...
colnames(abcd.data.combined.harmonized)[58]<-"General_p"
...
# linear mixed effects models associating F1 with F2
```{r}

summary(lmer(PRS_2~PRS_1+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized))
mod1<-lmer(PRS_2~PRS_1+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized)
tab_model(mod1,pred.labels = c("Intercept","PRS_1","Age","Sex"),dv.labels=c("PRS_2"),digits=2,digits.p=2,show.se=TRUE,show.stat=TRUE,string.stat = "t",string.se = "Std. Error",show.intercept=TRUE,show.ci = FALSE, p.adjust="none",p.style="scientific", file = paste("Associations_F1_F2_outcome_NEW.doc", sep = ""))
...
# linear mixed effects models associating psychopathology with F1 separately
```{r}

summary(lmer(General_p~PRS_1+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized))
mod1<-lmer(General_p~PRS_1+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized)
tab_model(mod1,pred.labels = c("Intercept","PRS_1","Age","Sex"),dv.labels=c("General_p"),digits=2,digits.p=2,show.se=TRUE,show.stat=TRUE,string.stat = "t",string.se = "Std. Error",show.intercept=TRUE,show.ci = FALSE, p.adjust="none",p.style="scientific", file = paste("Associations_F1_pfac_outcome_NEW.doc", sep = ""))
...
# linear mixed effects models associating psychopathology with F2 separately including meanFD
```{r}

summary(lmer(General_p~PRS_2+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized))
mod1<-lmer(General_p~PRS_2+interview_age+sex+(1|rel_family_id),data=abcd.data.combined.harmonized)
tab_model(mod1,pred.labels = c("Intercept","PRS_2","Age","Sex"),dv.labels=c("General_p"),digits=2,digits.p=2,show.se=TRUE,show.stat=TRUE,string.stat = "t",string.se = "Std. Error",show.intercept=TRUE,show.ci = FALSE, p.adjust="none",p.style="scientific", file = paste("Associations_F2_pfac_outcome_NEW.doc", sep = ""))
